...
| session suite: the union of both devices' session cases against fake devices (a Flipper shaped one with a real lock and foreground, a Pico shaped one with fixed guard values and a page key): handshake with retry, wholesale replacement, `BAD_VERSION`, drop on disconnect with nothing queued across it, a press only while connected and the guard allows, the bounded queue, the counters, the optional callbacks, no allocation | both devices, rewritten against the device surface (`SE5`) | `make test` |
