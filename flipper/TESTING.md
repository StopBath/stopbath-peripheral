...
and seen to fail (5 of 9 cases) before the wrapper was implemented.

`FE5` (2026-09-12) completes what the experiment left: known payloads against
published full matrix vectors, in `tests/test_remote_qr_vectors.c`, checking
whole matrices module for module against `tests/qr_published_vectors.h`. Those
vectors are the product path's own output, each reproduced exactly by
libqrencode, an encoder unrelated to the vendored one, and regenerated by
`scripts/generate_qr_vectors.sh` (which refuses to write on any mismatch); the
header is committed so the check needs no tool at test time. The other `FE5`
criterion, the distinct error the display shows for a refused payload, is in
`tests/test_remote_display_layout.c` ("an oversized payload shows a distinct
error not a code", and its NFC fallback variant). A measured consequence, that a
long guest SSID prefix pushes a real Wi-Fi payload past the version 3 ceiling and
the device falls back to NFC, is recorded in the evaluation log 4.4.
...
negative) before the builder was implemented. Those vectors were written from
the firmware's NDEF parser and Android's parser, not from a published source.

`FE6` (2026-09-12) supplies the published reference those lacked, in
`tests/test_remote_ndef_vectors.c` against `tests/ndef_published_vectors.h`. The
reference encoder is ndeflib (the `ndef` package from nfcpy), independent of this
repository's builder; the URI record is compared byte for byte and the Wi-Fi
credential as an order-independent set of attribute TLVs (attribute order within
a credential is not significant, and the two implementations differ on it). The
same suite proves the two structural `FE6` properties: the QR and the NFC record
for one page derive from a single value and cannot name different networks, and
no payload survives a builder refusing a later one. The header is generated by
`scripts/generate_ndef_vectors.py` (which needs `ndeflib`) and committed, so the
test needs no tool. The remaining `FE6` criterion, an iPhone opening the gallery
by a tap, is the author's hardware gate.
