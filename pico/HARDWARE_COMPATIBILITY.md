...
| Phase | Gate | Status |
|---|---|---|
| `SE3` | this device's link gate re-run on the image built from this tree: `KE4` (twenty cable pulls and a restart of each side against the development peer built from `../shared/`, `make -C ../shared peer`), then `KE6` (a session started and ended from the Pico against the real appliance, then the same from the dashboard with the Pico attached); recorded with the `stopbath-peripheral` commit the image was built from | CLEARED 2026-09-16 by the author on the image built from `3af80e3` (`dist/stopbath_pico.uf2`, 113664 bytes, sha256 beginning `98ffbcbe5f4770f2`, the tree clean at that commit). Against the real appliance: the link reconnected; a session was started from the Pico (the network came up) and ended from it (the network went down); the page changed from the Pico; the Pico was restarted and the link reconnected. The author states the `KE4` half (the cable pulls and the restart of each side against the peer built from `../shared/`), the dashboard started session with the Pico attached, and the journal check were all tested and passed in the same run; the pull count, the presses, the phone, and the appliance commit were not written down and are not claimed here beyond the author's word. The image is byte for byte the size and sections of the `KE6` image of 2026-09-15 (`d4c12a6` in the old repository) and of the `v0.3.0` build. |
